...
router.post("/:inventoryUid/items", async function(req: Request, res: Response) {
	try {
		...
		const itemUids = await Service.ReceiveInventoryItems(inventoryUid, items);
...
router.post("/:inventoryUid/reserve", async function(req: Request, res: Response) {
	try {
		const inventoryUid = req.params.inventoryUid as string;
		...
		const reservation = await Service.ReserveInventory(inventoryUid, quantity);
